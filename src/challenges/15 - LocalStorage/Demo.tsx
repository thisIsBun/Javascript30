...
const Demo = () => {
  ...
  const [lists, setLists] = useLocalStorage<TapasItem[]>(dataName, []);
  const [newItem, setNewItem] = useState<string>("");

  return (
    <div className="container-img text-ocean-950 flex h-screen w-screen flex-col items-center justify-center">
      <svg
        ...
        xmlSpace="preserve"
        className="mb-12 w-52 rounded-full bg-[rgba(0,0,0,0.1)] fill-white p-5"
      >
        <g>
      ...
      <div className="max-w-96 bg-[rgba(255,255,255,0.95)] p-5 shadow">
        <h2 className="text-center">LOCAL TAPAS</h2>
        <p></p>
        <ul className="plates">
            ...
            return (
              <li key={index}>
                <input
                  ...
                  checked={list.done}
                  onChange={(e) => {
                    const newLists = lists.map((data, i) => {
                      if (i === index) {
                        return { ...data, done: e.target.checked };
                      }
                      return data;
                    });
                    setLists(newLists);
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            const newLists = [...lists, { text: trimmed, done: false }];
            setLists(newLists);
            setNewItem("");
          }}
        >
      ...
      <DemoNoteButtons challengeId={challengeId || "15"} buttonName="Note" />
    </div>
  );
};
